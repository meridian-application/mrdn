...
var API_BASE_URL = "https://mrdn.onrender.com".replace(/\/+$/, "");
var SESSION_STORAGE_KEY = "meridian-api-session";
...
function getStoredToken() {
	var _getStorage;
	return ((_getStorage = getStorage$1()) === null || _getStorage === void 0 ? void 0 : _getStorage.getItem(SESSION_STORAGE_KEY)) || "";
}
function setStoredToken(token) {
	var _getStorage2;
	(_getStorage2 = getStorage$1()) === null || _getStorage2 === void 0 || _getStorage2.setItem(SESSION_STORAGE_KEY, token);
}
function clearStoredToken() {
	var _getStorage3;
	(_getStorage3 = getStorage$1()) === null || _getStorage3 === void 0 || _getStorage3.removeItem(SESSION_STORAGE_KEY);
}
...
async function requestBackend(path, options = {}) {
	var _options$token;
	const token = (_options$token = options.token) !== null && _options$token !== void 0 ? _options$token : getStoredToken();
...
async function loadBackendState(profile) {
	if (!isBackendConfigured()) return null;
	if (hasVKLaunchParams()) {
		const payload = await requestBackend("/api/auth/vk", {
			method: "POST",
			body: JSON.stringify({
	...
	try {
		return (await requestBackend("/api/session", {
			method: "GET",
			token
		})).state;
	} catch {
...
async function registerBackendUser(payload) {
	if (!isBackendConfigured()) return null;
...
async function loginBackendUser(email, password) {
	if (!isBackendConfigured()) return null;
...
async function saveBackendState(state) {
	...
	await requestBackend("/api/state", {
		method: "PUT",
...
async function logoutBackend() {
	...
	clearStoredToken();
	if (!token) return;
	await requestBackend("/api/logout", {
		method: "POST",
		token
	}).catch(() => null);
}
...
var sharedTherapyCompatibility = [
...
var articles = [
	{
		...
		label: "НОВОСТИ",
		excerpt: "Почему самостоятельное снижение доз инсулина может быть опасным и от чего зависит чувствительность к терапии.",
		readMinutes: 6,
	...
	},
	{
		...
		label: "НОВОСТИ",
		excerpt: "Разбираемся, когда белок действительно требует ограничений и как подбирать его безопасно.",
		readMinutes: 7,
	...
	},
	{
		...
		label: "НОВОСТИ",
		excerpt: "Сахар, вес, кожа, завтраки и сложные углеводы: коротко о популярных мифах.",
		readMinutes: 4,
	...
	},
	{
		...
		label: "НОВОСТИ",
		excerpt: "Как регулярная активность помогает контролировать глюкозу, снижать стресс и улучшать сон.",
		readMinutes: 2,
	...
	},
	{
		...
		label: "НОВОСТИ",
		excerpt: "Исследование показало, что более поздний завтрак может улучшать показатели глюкозы после еды.",
		readMinutes: 4,
...
function getAndroidBridge() {
	...
	return (_window$AndroidBridge = window.AndroidBridge) !== null && _window$AndroidBridge !== void 0 ? _window$AndroidBridge : null;
}
function getIosBridge() {
	var _window$webkit$messag, _window$webkit;
	if (!isClient()) return null;
	return (_window$webkit$messag = (_window$webkit = window.webkit) === null || _window$webkit === void 0 ? void 0 : _window$webkit.messageHandlers) !== null && _window$webkit$messag !== void 0 ? _window$webkit$messag : null;
}
function getWebBridge() {
	if (!isClient() || window.parent === window) return null;
...
function ensureListeners() {
	...
	const handler = (rawEvent) => {
		var _maybeMessageEvent$da, _eventPayload$type, _eventPayload$detail, _eventPayload$data, _eventPayload$detail2;
		const payload = (_maybeMessageEvent$da = rawEvent.data) !== null && _maybeMessageEvent$da !== void 0 ? _maybeMessageEvent$da : rawEvent.detail;
		if (!payload || typeof payload !== "object") return;
		const eventPayload = payload;
		const type = (_eventPayload$type = eventPayload.type) !== null && _eventPayload$type !== void 0 ? _eventPayload$type : (_eventPayload$detail = eventPayload.detail) === null || _eventPayload$detail === void 0 ? void 0 : _eventPayload$detail.type;
		const data = (_eventPayload$data = eventPayload.data) !== null && _eventPayload$data !== void 0 ? _eventPayload$data : (_eventPayload$detail2 = eventPayload.detail) === null || _eventPayload$detail2 === void 0 ? void 0 : _eventPayload$detail2.data;
		...
		if (data.error_type) {
			pending.reject(new Error(typeof data.error_data === "string" ? data.error_data : data.error_type));
			return;
		}
		pending.resolve(data);
	};
	window.addEventListener("message", handler);
	window.addEventListener("VKWebAppEvent", handler);
}
var listenersInitialized = false;
function prepareBridge() {
	if (listenersInitialized || !isClient()) return;
	ensureListeners();
	listenersInitialized = true;
}
function postBridgeEvent(method, params) {
...
function sendBridge(method, params = {}) {
	prepareBridge();
	if (!isEmbeddedRuntime()) return Promise.reject(/* @__PURE__ */ new Error("VK runtime is unavailable outside embedded mode."));
	...
	return new Promise((resolve, reject) => {
		...
		if (!postBridgeEvent(method, payload)) {
			pendingRequests.delete(requestId);
			reject(/* @__PURE__ */ new Error(`Unable to send ${method} through VK bridge.`));
			return;
		}
		window.setTimeout(() => {
			...
			reject(/* @__PURE__ */ new Error(`VK bridge timeout for ${method}.`));
		}, 4e3);
	});
}
function initializeVKBridge() {
	if (!initPromise) if (!isEmbeddedRuntime()) initPromise = Promise.resolve(null);
	else initPromise = sendBridge("VKWebAppInit").catch(() => null);
	return initPromise;
}
async function initVKMiniApp() {
	try {
		...
		const user = await sendBridge("VKWebAppGetUserInfo");
		return {
			id: user.id,
...
async function getVKStorageValues(keys) {
	try {
		var _response$keys;
		if (!isEmbeddedRuntime() || keys.length === 0) return {};
		await initializeVKBridge();
		const response = await sendBridge("VKWebAppStorageGet", { keys });
		return Object.fromEntries(((_response$keys = response.keys) !== null && _response$keys !== void 0 ? _response$keys : []).map((entry) => {
			var _entry$value;
			return [entry.key, (_entry$value = entry.value) !== null && _entry$value !== void 0 ? _entry$value : ""];
		}));
	} catch {
		return {};
	}
}
async function getVKStorageValue(key) {
	var _values$key;
	return (_values$key = (await getVKStorageValues([key]))[key]) !== null && _values$key !== void 0 ? _values$key : null;
}
async function setVKStorageValue(key, value) {
	try {
		if (!isEmbeddedRuntime()) return false;
		await initializeVKBridge();
		await sendBridge("VKWebAppStorageSet", {
			key,
			value
		});
		return true;
...
var STORAGE_KEY = "meridian-vk-mini-state";
var defaultState = {
	accounts: [],
	currentUserId: null
};
...
function parsePersistedState(raw) {
	...
	try {
		...
		if (parsed.currentUserId !== null && typeof parsed.currentUserId !== "string") return null;
		return parsed;
	} catch {
		return null;
	}
	...
	try {
		...
		if (vkData) {
			...
			if (parsed) {
				...
				if (storage) storage.setItem(STORAGE_KEY, vkData);
				return parsed;
			}
		}
		const storage = getStorage();
		const parsed = parsePersistedState(storage === null || storage === void 0 ? void 0 : storage.getItem(STORAGE_KEY));
		if (parsed) {
			memoryState = cloneState(parsed);
			return parsed;
		}
...
var surveyQuestions = [
	{
		key: "diabetesType",
		title: "Тип диабета",
		description: "Мы строго сохраняем конфиденциальность. Эти данные нужны для подбора безопасных рецептов.",
		kind: "choice",
		options: [
			"Сахарный Диабет 1 типа",
			"Сахарный Диабет 2 типа",
	...
	},
	{
		...
		description: "Отметьте продукты, которые следует исключить из рациона. Можно выбрать несколько пунктов или пропустить этот шаг.",
		kind: "nutrition",
		options: [
			"Соя",
			"Мед",
...
var metricPlaceholders = {
	glucose: "Например, 5.8",
	weight: "Например, 74.2",
	steps: "Например, 8432"
};
var MIN_SURVEY_AGE = 14;
var MAX_SURVEY_AGE = 120;
var VK_STORAGE_META_KEY = `${STORAGE_KEY}:meta`;
var VK_STORAGE_PART_KEY_PREFIX = `${STORAGE_KEY}:part:`;
var VK_STORAGE_CHUNK_SIZE = 3e3;
function isSurveyValuePresent(value) {
	if (Array.isArray(value)) return value.length > 0;
	return value !== void 0 && value !== null && value !== "";
}
function isSurveyQuestionAnswered(question, survey) {
	if (question.kind === "nutrition") return isSurveyValuePresent(survey[question.key]) || isSurveyValuePresent(survey[question.secondaryKey]);
	return isSurveyValuePresent(survey[question.key]);
}
function getAnsweredSurveyCount(survey) {
...
function getAccountCompletenessScore(account) {
	return getAnsweredSurveyCount(account.survey) * 3 + (account.onboardingComplete || isSurveyComplete(account.survey) ? 50 : 0) + account.metrics.length * 2 + account.favorites.length + (account.fullName.trim() ? 2 : 0) + (account.region.trim() ? 1 : 0) + (account.password ? 1 : 0);
}
function pickPreferredAccount(left, right) {
...
function mergePersistedStates(localState, remoteState) {
	var _currentUserCandidate;
	...
	const accounts = Array.from(accountMap.values()).sort((left, right) => new Date(left.createdAt).getTime() - new Date(right.createdAt).getTime());
	const accountIds = new Set(accounts.map((account) => account.id));
	return {
		accounts,
		currentUserId: (_currentUserCandidate = [remoteState.currentUserId, localState.currentUserId].filter(Boolean).map((id) => {
			var _resolvedIds$get;
			return (_resolvedIds$get = resolvedIds.get(id)) !== null && _resolvedIds$get !== void 0 ? _resolvedIds$get : id;
		}).find((id) => accountIds.has(id))) !== null && _currentUserCandidate !== void 0 ? _currentUserCandidate : null
	};
}
function isSameState(left, right) {
	return JSON.stringify(left) === JSON.stringify(right);
}
function buildRemoteChunkKeys(parts) {
	return Array.from({ length: parts }, (_, index) => `${VK_STORAGE_PART_KEY_PREFIX}${index}`);
}
function splitPersistedPayload(value, chunkSize = VK_STORAGE_CHUNK_SIZE) {
	const parts = [];
	for (let index = 0; index < value.length; index += chunkSize) parts.push(value.slice(index, index + chunkSize));
	return parts.length ? parts : [""];
}
function parseRemoteStateMeta(raw) {
	if (!raw) return null;
	try {
		const parsed = JSON.parse(raw);
		if (!Number.isInteger(parsed.parts) || parsed.parts < 1) return null;
		return parsed;
	} catch {
		return null;
	}
...
async function loadRemotePersistedState() {
	...
	if (meta) {
		...
		return parsePersistedState(chunkKeys.map((key) => {
			var _chunkMap$key;
			return (_chunkMap$key = chunkMap[key]) !== null && _chunkMap$key !== void 0 ? _chunkMap$key : "";
		}).join(""));
	}
	return parsePersistedState(await getVKStorageValue(STORAGE_KEY));
}
async function saveRemotePersistedState(state) {
	...
	let saved = await setVKStorageValue(VK_STORAGE_META_KEY, JSON.stringify({ parts: chunks.length }));
	for (const [index, chunk] of chunks.entries()) saved = await setVKStorageValue(`${VK_STORAGE_PART_KEY_PREFIX}${index}`, chunk) && saved;
	if (previousMeta && previousMeta.parts > chunks.length) for (let index = chunks.length; index < previousMeta.parts; index += 1) saved = await setVKStorageValue(`${VK_STORAGE_PART_KEY_PREFIX}${index}`, "") && saved;
	saved = await setVKStorageValue("meridian-vk-mini-state", chunks.length === 1 ? payload : "") && saved;
	return saved;
}
async function restoreVkCurrentUser(state, profile) {
	if (!(profile === null || profile === void 0 ? void 0 : profile.id) || state.currentUserId) return state;
...
function App() {
	var _state$accounts$find, _recipes$find, _articles$find, _ref, _currentUser$fullName, _currentUser$favorite, _favoriteRecipes$, _recommendedRecipes$, _currentUser$metrics, _currentUser$metrics2, _currentUser$metrics3, _weightEntries$, _weightEntries$2, _stepsEntries$, _currentUser$metrics$;
	const [state, setState] = (0, import_react.useState)(defaultState);
	const [authView, setAuthView] = (0, import_react.useState)("launch");
	const [activeTab, setActiveTab] = (0, import_react.useState)("home");
	const [homeSection, setHomeSection] = (0, import_react.useState)("recommended");
	const [recipeCatalogTab, setRecipeCatalogTab] = (0, import_react.useState)("Все рецепты");
	const [searchQuery, setSearchQuery] = (0, import_react.useState)("");
	const deferredSearch = (0, import_react.useDeferredValue)(searchQuery);
	const [selectedRecipeId, setSelectedRecipeId] = (0, import_react.useState)(null);
	const [selectedArticleId, setSelectedArticleId] = (0, import_react.useState)(null);
	const [metricsRange, setMetricsRange] = (0, import_react.useState)("7d");
	const [loginForm, setLoginForm] = (0, import_react.useState)({
		email: "",
		password: ""
	});
	...
	const [surveyMode, setSurveyMode] = (0, import_react.useState)("intro");
	const [showSurveyCompletionScreen, setShowSurveyCompletionScreen] = (0, import_react.useState)(false);
	const [surveyStepIndex, setSurveyStepIndex] = (0, import_react.useState)(0);
	const [surveyDraft, setSurveyDraft] = (0, import_react.useState)({});
	const [surveyError, setSurveyError] = (0, import_react.useState)(null);
	const [metricFormOpen, setMetricFormOpen] = (0, import_react.useState)(false);
	const [metricDraft, setMetricDraft] = (0, import_react.useState)({
		type: "glucose",
		value: "",
		note: ""
	});
	const [metricError, setMetricError] = (0, import_react.useState)(null);
	const [profileEditorOpen, setProfileEditorOpen] = (0, import_react.useState)(false);
	const [profileDraft, setProfileDraft] = (0, import_react.useState)({
		fullName: "",
		region: "Москва"
	});
	const [vkUser, setVkUser] = (0, import_react.useState)(null);
	const [storageHydrated, setStorageHydrated] = (0, import_react.useState)(false);
	(0, import_react.useEffect)(() => {
		let cancelled = false;
		(async () => {
			const profile = await initVKMiniApp();
			if (cancelled) return;
			if (profile) setVkUser(profile);
			const backendState = await loadBackendState(profile).catch(() => null);
			const resolvedState = await restoreVkCurrentUser(backendState !== null && backendState !== void 0 ? backendState : await loadState(), profile);
			if (cancelled) return;
	...
	const currentUser = (_state$accounts$find = state.accounts.find((account) => account.id === state.currentUserId)) !== null && _state$accounts$find !== void 0 ? _state$accounts$find : null;
	const currentUserOnboardingComplete = currentUser ? currentUser.onboardingComplete || isSurveyComplete(currentUser.survey) : false;
	const selectedRecipe = (_recipes$find = recipes.find((recipe) => recipe.id === selectedRecipeId)) !== null && _recipes$find !== void 0 ? _recipes$find : null;
	const selectedArticle = (_articles$find = articles.find((article) => article.id === selectedArticleId)) !== null && _articles$find !== void 0 ? _articles$find : null;
	const greetingName = (_ref = (_currentUser$fullName = currentUser === null || currentUser === void 0 ? void 0 : currentUser.fullName) !== null && _currentUser$fullName !== void 0 ? _currentUser$fullName : buildVKName(vkUser)) !== null && _ref !== void 0 ? _ref : "друг";
	const availableRecipeIds = new Set(recipes.map((recipe) => recipe.id));
	const favoriteIds = new Set(((_currentUser$favorite = currentUser === null || currentUser === void 0 ? void 0 : currentUser.favorites) !== null && _currentUser$favorite !== void 0 ? _currentUser$favorite : []).filter((recipeId) => availableRecipeIds.has(recipeId)));
	const favoritesCount = favoriteIds.size;
	const stateRef = (0, import_react.useRef)(state);
	const persistStateSnapshot = async (nextState, awaitRemote = false) => {
		await saveState(nextState);
		if (!storageHydrated) return true;
		const remoteRequest = Promise.all([saveRemotePersistedState(nextState), saveBackendState(nextState)]).then((results) => results.every(Boolean));
		if (awaitRemote) return remoteRequest;
		return true;
	};
	const commitState = async (nextState, options = {}) => {
		stateRef.current = nextState;
		setState(nextState);
		return persistStateSnapshot(nextState, Boolean(options.awaitRemote));
	};
	const updatePersistedState = async (transform, options = {}) => {
		const nextState = transform(stateRef.current);
		await commitState(nextState, options);
		return nextState;
	};
	const syncStateFromRemote = async () => {
		...
		if (!isSameState(stateRef.current, mergedState)) await commitState(mergedState);
		return mergedState;
	};
	...
	const recommendedRecipes = [...searchableRecipes].sort((left, right) => {
		var _currentUser$survey, _currentUser$survey2;
		return getRecipeScore(right, (_currentUser$survey = currentUser === null || currentUser === void 0 ? void 0 : currentUser.survey) !== null && _currentUser$survey !== void 0 ? _currentUser$survey : {}) - getRecipeScore(left, (_currentUser$survey2 = currentUser === null || currentUser === void 0 ? void 0 : currentUser.survey) !== null && _currentUser$survey2 !== void 0 ? _currentUser$survey2 : {});
	});
	const favoriteRecipes = recommendedRecipes.filter((recipe) => favoriteIds.has(recipe.id));
	const heroRecipe = homeSection === "favorites" ? (_favoriteRecipes$ = favoriteRecipes[0]) !== null && _favoriteRecipes$ !== void 0 ? _favoriteRecipes$ : null : (_recommendedRecipes$ = recommendedRecipes[0]) !== null && _recommendedRecipes$ !== void 0 ? _recommendedRecipes$ : null;
	const catalogSourceRecipes = homeSection === "favorites" ? favoriteRecipes.slice(heroRecipe ? 1 : 0) : searchableRecipes.filter((recipe) => recipe.id !== (heroRecipe === null || heroRecipe === void 0 ? void 0 : heroRecipe.id));
	const catalogRecipes = homeSection === "favorites" ? catalogSourceRecipes : catalogSourceRecipes.filter((recipe) => matchesRecipeCatalogTab(recipe, recipeCatalogTab));
	const glucoseEntries = ((_currentUser$metrics = currentUser === null || currentUser === void 0 ? void 0 : currentUser.metrics) !== null && _currentUser$metrics !== void 0 ? _currentUser$metrics : []).filter((entry) => entry.type === "glucose");
	const weightEntries = ((_currentUser$metrics2 = currentUser === null || currentUser === void 0 ? void 0 : currentUser.metrics) !== null && _currentUser$metrics2 !== void 0 ? _currentUser$metrics2 : []).filter((entry) => entry.type === "weight");
	const stepsEntries = ((_currentUser$metrics3 = currentUser === null || currentUser === void 0 ? void 0 : currentUser.metrics) !== null && _currentUser$metrics3 !== void 0 ? _currentUser$metrics3 : []).filter((entry) => entry.type === "steps");
	...
	const latestWeight = (_weightEntries$ = weightEntries[0]) !== null && _weightEntries$ !== void 0 ? _weightEntries$ : null;
	const previousWeight = (_weightEntries$2 = weightEntries[1]) !== null && _weightEntries$2 !== void 0 ? _weightEntries$2 : null;
	const latestSteps = (_stepsEntries$ = stepsEntries[0]) !== null && _stepsEntries$ !== void 0 ? _stepsEntries$ : null;
	const chartPoints = buildChartPoints(recentGlucoseEntries, metricsRange);
	const hasGlucoseData = recentGlucoseEntries.length > 0;
	const hasMetricData = ((_currentUser$metrics$ = currentUser === null || currentUser === void 0 ? void 0 : currentUser.metrics.length) !== null && _currentUser$metrics$ !== void 0 ? _currentUser$metrics$ : 0) > 0;
	const glucoseStatus = !hasGlucoseData ? "Нет данных" : averageGlucose <= 6 ? "Норма" : averageGlucose <= 7.2 ? "Контроль" : "Внимание";
	(0, import_react.useEffect)(() => {
		initVKMiniApp().then((profile) => {
			if (!profile) return;
			setVkUser(profile);
		});
	}, []);
	...
	(0, import_react.useEffect)(() => {
		...
		const save = async () => {
			...
			saveRemotePersistedState(state);
	...
	(0, import_react.useEffect)(() => {
		if (!vkUser) return;
		const suggestedName = buildVKName(vkUser);
		if (!suggestedName) return;
		setRegisterForm((current) => current.fullName ? current : {
			...current,
	...
	(0, import_react.useEffect)(() => {
		...
		if (currentUserOnboardingComplete) {
			if (!currentUser.onboardingComplete) syncCurrentUser((user) => ({
				...user,
	...
	const syncCurrentUser = (transform) => {
		updatePersistedState((current) => {
			if (!current.currentUserId) return current;
			return {
				...current,
				accounts: current.accounts.map((account) => account.id === current.currentUserId ? transform(account) : account)
			};
		});
	};
	const handleRegistration = async (event) => {
		...
		setAuthError(null);
		const fullName = registerForm.fullName.trim() || buildVKName(vkUser) || "Александр Петров";
		const email = normalizeEmail(registerForm.email);
		if (!fullName || !email || !registerForm.password || !registerForm.confirmPassword) {
		...
		if (isBackendConfigured()) try {
			const backendState = await registerBackendUser({
				fullName,
				email,
				password: registerForm.password,
				survey: surveyDraft,
				onboardingComplete: isSurveyComplete(surveyDraft)
			});
			if (backendState) {
				var _buildVKName;
				await commitState(backendState, { awaitRemote: false });
				setRegisterForm({
					fullName: (_buildVKName = buildVKName(vkUser)) !== null && _buildVKName !== void 0 ? _buildVKName : "",
					email: "",
					password: "",
					confirmPassword: ""
				});
				setSurveyMode("complete");
				setSurveyStepIndex(surveyQuestions.length);
				setShowSurveyCompletionScreen(isSurveyComplete(surveyDraft));
				return;
			}
		} catch (error) {
			setAuthError(error instanceof Error ? error.message : "Не удалось зарегистрироваться.");
			return;
		}
		const syncedState = await syncStateFromRemote();
		if (syncedState.accounts.some((account) => normalizeEmail(account.email) === email)) {
			setAuthError("Аккаунт с такой почтой уже существует.");
			return;
		}
		const user = {
			id: crypto.randomUUID(),
			vkId: (vkUser === null || vkUser === void 0 ? void 0 : vkUser.id) ? String(vkUser.id) : void 0,
			fullName,
			email,
			password: registerForm.password,
			region: "Москва",
		...
		await commitState({
			accounts: [...syncedState.accounts, user],
			currentUserId: user.id
		}, { awaitRemote: true });
		setRegisterForm({
			fullName: (_buildVKName2 = buildVKName(vkUser)) !== null && _buildVKName2 !== void 0 ? _buildVKName2 : "",
			email: "",
			password: "",
			confirmPassword: ""
		});
	...
	const handleLogin = async (event) => {
		...
		setAuthError(null);
		if (isBackendConfigured()) try {
			const backendState = await loginBackendUser(loginForm.email, loginForm.password);
			if (backendState) {
				await commitState(backendState, { awaitRemote: false });
				setLoginForm({
					email: "",
		...
		const account = syncedState.accounts.find((item) => normalizeEmail(item.email) === normalizedEmail);
		if (!account || account.password !== loginForm.password) {
			setAuthError("Неверная почта или пароль.");
			return;
		}
		const vkId = (vkUser === null || vkUser === void 0 ? void 0 : vkUser.id) ? String(vkUser.id) : void 0;
		const updatedAccounts = syncedState.accounts.map((item) => item.id === account.id ? {
			...item,
			vkId: vkId && item.vkId !== vkId ? vkId : item.vkId
		} : item);
		await commitState({
			...syncedState,
	...
	const updateSurveyDraft = (key, value) => {
		const nextSurvey = {
			...surveyDraft,
			[key]: value
		};
		...
		setSurveyError(null);
		if (currentUser) syncCurrentUser((user) => ({
			...user,
			survey: nextSurvey
		}));
	};
	const toggleSurveyDraftValue = (key, option) => {
		const currentValues = Array.isArray(surveyDraft[key]) ? surveyDraft[key] : [];
		updateSurveyDraft(key, currentValues.includes(option) ? currentValues.filter((value) => value !== option) : [...currentValues, option]);
	};
	const toggleOptionalChoiceValue = (key, value) => {
		updateSurveyDraft(key, surveyDraft[key] === value ? void 0 : value);
	};
	...
	const handlePreRegistrationSurveyContinue = () => {
		const question = surveyQuestions[surveyStepIndex];
		if (!question) return;
		const answer = surveyDraft[question.key];
		if (question.kind === "number") {
	...
	const handlePreRegistrationSurveyBack = () => {
	...
	const toggleFavorite = (recipeId) => {
		syncCurrentUser((user) => {
			const nextFavorites = user.favorites.includes(recipeId) ? user.favorites.filter((id) => id !== recipeId) : [...user.favorites, recipeId];
			return {
				...user,
	...
	const handleSaveMetric = (event) => {
		...
		const nextMetric = {
			...
			note: metricDraft.note.trim() || (metricDraft.type === "glucose" ? "Новая запись сахара" : metricDraft.type === "weight" ? "Контроль веса" : "Ежедневная активность"),
			timestamp: (/* @__PURE__ */ new Date()).toISOString()
		};
		syncCurrentUser((user) => ({
			...user,
			metrics: [nextMetric, ...user.metrics].sort((left, right) => new Date(right.timestamp).getTime() - new Date(left.timestamp).getTime())
		}));
		setMetricDraft({
			type: metricDraft.type,
			value: "",
	...
	let content;
	if (!currentUser) if (!storageHydrated) content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(StorageLoadingScreen, {});
	else if (authView === "launch") content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LaunchScreen, {
		onLogin: () => {
			setAuthError(null);
			setAuthView("login");
		},
		onRegister: () => {
			...
			setSurveyMode("intro");
			setSurveyError(null);
			setAuthView("preSurvey");
		}
	});
	else if (authView === "preSurvey") {
		const currentQuestion = surveyQuestions[surveyStepIndex];
		if (surveyMode === "intro") content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SurveyIntro, {
			...
			content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SurveyQuestionScreen, {
				title: currentQuestion.title,
				description: currentQuestion.description,
				onBack: handlePreRegistrationSurveyBack,
				error: surveyError,
				footer: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
					variant: "dark",
					onClick: handlePreRegistrationSurveyContinue,
					children: "Подтвердить"
				}),
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "option-list",
					children: currentQuestion.options.map((option) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(OptionCard, {
			...
			const selectedProducts = Array.isArray(surveyDraft[currentQuestion.key]) ? surveyDraft[currentQuestion.key] : [];
			const selectedRestriction = typeof surveyDraft[currentQuestion.secondaryKey] === "string" ? surveyDraft[currentQuestion.secondaryKey] : void 0;
			content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SurveyQuestionScreen, {
				title: currentQuestion.title,
				description: currentQuestion.description,
				onBack: handlePreRegistrationSurveyBack,
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "survey-stack",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "survey-tag-list",
						children: currentQuestion.options.map((option) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							className: classNames("survey-tag", selectedProducts.includes(option) && "is-selected"),
							onClick: () => toggleSurveyDraftValue("excludedProducts", option),
							type: "button",
							children: option
						}, option))
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "survey-stack survey-stack--section",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
							className: "survey-section-title",
							children: currentQuestion.secondaryTitle
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "option-list option-list--compact",
							children: currentQuestion.secondaryOptions.map((option) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(OptionCard, {
		...
		} else content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SurveyQuestionScreen, {
			title: currentQuestion.title,
			description: currentQuestion.description,
			onBack: handlePreRegistrationSurveyBack,
			error: surveyError,
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Field, {
				...
				onChange: (event) => {
					...
					updateSurveyDraft("age", digits ? Number(digits) : void 0);
				},
				type: "number",
				inputMode: "numeric",
				min: MIN_SURVEY_AGE,
	...
	} else if (authView === "login") content = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(AuthScreen, {
		title: "Авторизация",
		subtitle: "Ваш надежный партнер в питании",
		error: authError,
		onBack: () => {
			setAuthError(null);
			setAuthView("launch");
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Field, {
			...
			icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LockIcon, {}),
			autoComplete: "current-password"
		})]
	});
	else content = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(AuthScreen, {
		title: "Регистрация",
		subtitle: "Создайте профиль и получите персональные рекомендации",
		error: authError,
		onBack: () => {
			setAuthError(null);
			setAuthView("preSurvey");
			setSurveyMode("questions");
			setSurveyStepIndex(surveyQuestions.length - 1);
		},
		onSubmit: handleRegistration,
		actionLabel: "Завершить регистрацию",
		footerLabel: "Уже зарегистрированы?",
		footerAction: "Войти",
		onFooterClick: () => {
			setAuthError(null);
			setAuthView("login");
	...
	else if (showSurveyCompletionScreen) content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SurveyDone, { onStart: () => {
		setShowSurveyCompletionScreen(false);
		setActiveTab("home");
	} });
	...
	} else if (selectedRecipe) content = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "app-layout",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "screen-content screen-content--detail",
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(DetailLayout, {
				onBack: () => setSelectedRecipeId(null),
				title: "Ваш рецепт",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(RecipePreviewCard, {
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
								className: "recipe-eyebrow",
								children: selectedRecipe.tags.join(" • ")
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: selectedRecipe.title }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
						className: "detail-section",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", { children: "Ингредиенты" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("ul", {
							className: "detail-list",
							children: selectedRecipe.ingredients.map((ingredient) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("li", { children: ingredient }, ingredient))
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
						className: "detail-section",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", { children: "Приготовление" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("ol", {
							className: "detail-list detail-list--ordered",
							children: selectedRecipe.steps.map((step) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("li", { children: step }, step))
						})]
					})
				]
			})
			...
			activeTab,
			onChange: (tab) => (0, import_react.startTransition)(() => {
				setActiveTab(tab);
				setSelectedArticleId(null);
				setSelectedRecipeId(null);
			})
	...
	else if (selectedArticle) content = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "app-layout",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "screen-content screen-content--detail",
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DetailLayout, {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							...
							children: selectedArticle.label
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: selectedArticle.title }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "article-highlight",
							children: selectedArticle.highlight
						}),
						selectedArticle.body.map((paragraph) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: paragraph }, paragraph))
					]
				})
			})
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BottomNavigation, {
		...
		var _latestSteps$value, _currentUser$metrics4, _currentUser$survey$d, _currentUser$survey$h;
		content = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
						activeTab === "home" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
								className: "screen-header",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									className: "eyebrow",
									children: "ВАШ ПЕРСОНАЛЬНЫЙ ХАБ"
									...
									name: greetingName,
									photo: vkUser === null || vkUser === void 0 ? void 0 : vkUser.photo_200
								})]
							}),
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
									className: classNames("section-switcher__item", homeSection === "recommended" && "is-active"),
									onClick: () => setHomeSection("recommended"),
									type: "button",
									children: "Рекомендовано"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
									className: classNames("section-switcher__item", homeSection === "favorites" && "is-active"),
									onClick: () => setHomeSection("favorites"),
									type: "button",
									children: "Избранное"
								})]
							}),
							heroRecipe ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
								className: "hero-recipe-section",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SectionHeading, {
									title: homeSection === "favorites" ? "Избранные рецепты" : "Сегодня для вас",
									subtitle: homeSection === "favorites" ? favoritesCount ? `У вас ${formatSavedRecipes(favoritesCount)}.` : "Сохраняйте блюда сердечком, и они появятся в личной подборке." : heroRecipe.focus
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RecipePreviewCard, {
									recipe: heroRecipe,
									isFavorite: favoriteIds.has(heroRecipe.id),
									compact: false,
									onToggleFavorite: () => toggleFavorite(heroRecipe.id),
									onOpen: () => (0, import_react.startTransition)(() => {
										setSelectedRecipeId(heroRecipe.id);
									})
								})]
							}), homeSection === "recommended" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										className: "chip-row chip-row--after-hero catalog-tabs",
										role: "tablist",
										children: recipeCatalogTabs.map((tab) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
											type: "button",
											role: "tab",
											"aria-selected": recipeCatalogTab === tab,
											className: classNames("filter-chip", recipeCatalogTab === tab && "is-active"),
											onClick: () => setRecipeCatalogTab(tab),
											children: tab
										}, tab))
									}),
									catalogRecipes.length ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("section", {
										className: "recipe-grid",
										children: catalogRecipes.map((recipe) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RecipePreviewCard, {
											recipe,
							...
							}) }) : catalogRecipes.length ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
								className: "catalog-section",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SectionHeading, {
							...
							}) : null] }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EmptyState, {
								title: homeSection === "favorites" ? "В избранном пока пусто." : "По вашему запросу пока ничего не найдено.",
								description: homeSection === "favorites" ? "Сохраняйте рецепты сердечком, и они будут всегда под рукой." : "Попробуйте изменить запрос или очистить поиск, чтобы увидеть все рецепты."
							})
						] }) : null,
						activeTab === "feed" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
							className: "feed-hero",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "feed-hero__badge",
								children: "НОВОСТИ"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Свежие новости и полезные обновления без лишних разделов." })]
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "article-list",
							children: articles.map((article) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [formatReadMinutes(article.readMinutes), " чтения"] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronRightIcon, {})]
									})
								]
							}, article.id))
						})] }) : null,
						activeTab === "metrics" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
								className: "screen-header screen-header--compact",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
									className: "screen-title",
									children: "Метрики"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									className: "screen-subtitle",
									children: formatLongDate((/* @__PURE__ */ new Date()).toISOString())
								})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
									variant: "soft",
									className: "metrics-add-button",
									onClick: () => setMetricFormOpen(true),
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PlusIcon, {}), "Запись"]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "metrics-highlight-grid",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
									className: "metric-highlight metric-highlight--soft",
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "сегодня" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: glucoseStatus })]
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
									className: "metric-highlight metric-highlight--strong",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "средний сахар" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: hasGlucoseData ? averageGlucose.toFixed(1) : "—" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: hasGlucoseData ? "ммоль/л" : "Добавьте запись" })
									]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "Глюкоза крови" }) }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								}), hasGlucoseData ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: classNames("metrics-chart", metricsRange === "30d" && "metrics-chart--30d"),
									children: chartPoints.map((point, index) => {
										const showLabel = metricsRange === "7d" || index === 0 || index === chartPoints.length - 1 || index % 5 === 4;
										return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											className: "metrics-chart__item",
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
												className: "metrics-chart__bar-track",
												children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
													className: classNames("metrics-chart__bar", point.isAccent && "is-accent"),
													style: { height: `${point.height}%` }
												})
											}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
												className: classNames("metrics-chart__label", !showLabel && "is-hidden"),
												children: showLabel ? point.label : ""
											})]
										}, `${point.label}-${index}`);
									})
								}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EmptyState, {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
									...
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
											...
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ScaleIcon, {})
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Вес" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: latestWeight ? `${latestWeight.value.toFixed(1)} кг` : "—" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: latestWeight && previousWeight ? `${formatSigned(latestWeight.value - previousWeight.value)} кг за период` : "Добавьте первую запись веса" })
									]
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
									className: "stats-card",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
											className: "stats-card__icon stats-card__icon--warm",
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ActivityIcon, {})
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Активность" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: latestSteps ? `${Math.round(latestSteps.value)}` : "—" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: latestSteps ? `${getRussianPlural(Math.round(latestSteps.value), [
											"шаг",
											"шага",
											"шагов"
										])} за день` : "Добавьте запись активности" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
											className: "progress-line",
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: { width: `${Math.min(100, ((_latestSteps$value = latestSteps === null || latestSteps === void 0 ? void 0 : latestSteps.value) !== null && _latestSteps$value !== void 0 ? _latestSteps$value : 0) / 1e4 * 100)}%` } })
										})
									]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
								className: "metric-log",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SectionHeading, {
									title: "Последние записи",
									subtitle: "Все свежие показатели пользователя"
								}), hasMetricData ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "metric-log__list",
									children: ((_currentUser$metrics4 = currentUser.metrics) !== null && _currentUser$metrics4 !== void 0 ? _currentUser$metrics4 : []).slice(0, 6).map((entry) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
										className: classNames("metric-log__item", `metric-log__item--${entry.type}`),
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											className: "metric-log__content",
											children: [
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: metricEntryTitle(entry) }),
												/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
													formatMetricDate(entry.timestamp),
													", ",
													formatMetricTime(entry.timestamp)
												] }),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
													className: "metric-log__note",
													children: entry.note
												})
											]
										}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", {
											className: "metric-log__value",
											children: formatMetricValue(entry)
										})]
									}, entry.id))
								}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EmptyState, {
						...
						activeTab === "profile" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "profile-hero__identity",
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Avatar, {
										...
										children: "ПРЕМИУМ СТАТУС"
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: currentUser.fullName })] })]
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "profile-meta",
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "ID пациента" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: buildPatientId(currentUser.id) })] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Регион" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: currentUser.region })] })]
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									...
									children: "Настройки и предпочтения"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "profile-card-list",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
											...
											children: [
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
													...
													children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ProfileDataIcon, {})
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "Личные данные" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Профиль, контакты, регион" })] }),
										...
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
											...
											children: [
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
													className: "profile-card__icon",
													children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SavedIcon, {})
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "Избранное" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: formatSavedRecipes(favoritesCount) })] }),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronRightIcon, {})
											]
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
											...
											children: [
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
													className: "profile-card__icon",
													children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(StatsIcon, {})
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "Настройки здоровья" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Цели, параметры, устройства" })] }),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronRightIcon, {})
											]
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
											...
											children: [
												...
												/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "Уведомления" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Пуши, email, напоминания" })] }),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronRightIcon, {})
											]
										})
									]
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SectionHeading, {
									...
									className: "health-summary__grid",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SummaryTile, {
											label: "Тип диабета",
											value: (_currentUser$survey$d = currentUser.survey.diabetesType) !== null && _currentUser$survey$d !== void 0 ? _currentUser$survey$d : "—"
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SummaryTile, {
											label: "Терапия",
											value: shortenTherapy(currentUser.survey.therapy)
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SummaryTile, {
											label: "Возраст",
										...
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SummaryTile, {
											label: "Гипогликемии",
											value: (_currentUser$survey$h = currentUser.survey.hypoglycemia) !== null && _currentUser$survey$h !== void 0 ? _currentUser$survey$h : "—"
										})
									]
								})]
							}),
				...
				metricFormOpen ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ModalSurface, {
					title: "Новая запись",
					onClose: () => setMetricFormOpen(false),
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								...
								children: [
									...
									className: classNames("segmented-control__item", metricDraft.type === type && "is-active"),
									onClick: () => setMetricDraft((current) => ({
										...current,
										type
									})),
									type: "button",
									children: type === "glucose" ? "Сахар" : type === "weight" ? "Вес" : "Шаги"
								}, type))
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Field, {
								label: "Значение",
								...
								onChange: (event) => setMetricDraft((current) => ({
									...current,
							...
							metricError ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
								className: "form-error",
								children: metricError
							}) : null,
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
								type: "submit",
								children: "Сохранить"
				...
				profileEditorOpen ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ModalSurface, {
					title: "Личные данные",
...
function StorageLoadingScreen() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("section", {
		className: "launch-screen launch-screen--loading",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "launch-screen__content",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BrandLogo, {}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: "МЕРИДИАН" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Восстанавливаем ваш профиль и вход..." })
			]
		})
	});
}
function LaunchScreen({ onLogin, onRegister }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BrandLogo, {}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: "МЕРИДИАН" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Ваш надежный партнер в питании" })
			]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
				...
				variant: "soft",
				onClick: onRegister,
				children: "Я здесь впервые"
			})]
		})]
	});
}
function AuthScreen({ title, subtitle, children, error, actionLabel, footerLabel, footerAction, onBack, onSubmit, onFooterClick }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		className: "auth-screen",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "auth-screen__hero",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
				className: "round-back auth-back-button",
				onClick: onBack,
				type: "button",
				"aria-label": "Назад",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowLeftIcon, {})
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: title }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: subtitle })] })]
...
function SurveyIntro({ name, onContinue, onBack }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		className: "survey-screen survey-screen--intro",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
				className: "round-back round-back--black",
				onClick: onBack,
				type: "button",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowLeftIcon, {})
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "survey-intro-copy",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: name ? `Добро пожаловать, ${name}!` : "Добро пожаловать" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Предлагаем пройти короткую анкету, чтобы определить ваш профиль здоровья и собрать первую подборку рецептов." })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
				variant: "dark",
				onClick: onContinue,
...
function SurveyQuestionScreen({ title, description, children, onBack, footer, error }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					children,
					error ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "form-error form-error--spaced",
						children: error
					}) : null
				]
			}),
...
function SurveyDone({ onStart }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		className: "survey-screen survey-screen--done",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "survey-intro-copy",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: "СПАСИБО!" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Мы подготовили для вас первую подборку рецептов на основе ваших данных. Помните: баланс — это ваш МЕРИДИАН." })]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
			className: "survey-done__button",
			onClick: onStart,
			children: ["Начать", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChevronRightIcon, {})]
		})]
	});
}
function DetailLayout({ children, title, onBack }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
...
function RecipePreviewCard({ recipe, isFavorite, compact, onToggleFavorite, onOpen }) {
	var _recipe$cardImagePosi;
	const style = {
		"--recipe-bg": recipe.palette.background,
		"--recipe-accent": recipe.palette.accent,
		"--recipe-glow": recipe.palette.imageGlow,
		"--recipe-image-position": (_recipe$cardImagePosi = recipe.cardImagePosition) !== null && _recipe$cardImagePosi !== void 0 ? _recipe$cardImagePosi : "center 18%"
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
		className: classNames("recipe-card", compact ? "recipe-card--compact" : "recipe-card--hero", !recipe.cardImage && "recipe-card--placeholder", recipe.cardImage && "recipe-card--asset"),
		style,
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
			...
			type: "button",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: classNames("recipe-card__media", !recipe.cardImage && "recipe-card__media--placeholder"),
				children: [recipe.cardImage ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
					className: "recipe-card__image",
					src: recipe.cardImage,
					alt: recipe.title,
					loading: "lazy"
				}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "recipe-card__placeholder-note",
					children: "Фото скоро будет"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "recipe-card__category",
					children: recipe.category
				})]
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: classNames("recipe-card__body", recipe.cardImage && "recipe-card__body--image"),
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "recipe-card__eyebrow",
						children: recipe.tags.slice(0, 2).join(" • ")
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", { children: recipe.title }),
					!compact ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: recipe.subtitle }) : null,
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "recipe-card__stats",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["ГИ ", recipe.gi] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["ХЕ ", recipe.xe] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [recipe.calories, " ккал"] })
						]
					})
				]
			})]
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("nav", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
				...
				type: "button",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(HomeIcon, { active: activeTab === "home" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Главная" })]
			}, "home"),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
				className: classNames("bottom-nav__item", activeTab === "feed" && "is-active"),
				onClick: () => onChange("feed"),
				type: "button",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FeedIcon, { active: activeTab === "feed" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Лента" })]
			}, "feed"),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
				className: classNames("bottom-nav__item", activeTab === "metrics" && "is-active"),
				onClick: () => onChange("metrics"),
				type: "button",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartIcon, { active: activeTab === "metrics" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Показатели" })]
			}, "metrics"),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
				className: classNames("bottom-nav__item", activeTab === "profile" && "is-active"),
				onClick: () => onChange("profile"),
				type: "button",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ProfileIcon, { active: activeTab === "profile" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "Профиль" })]
			}, "profile")
		]
	});
}
function ModalSurface({ title, onClose, children }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "modal-surface__header",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: title }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					type: "button",
					onClick: onClose,
					"aria-label": "Закрыть",
					children: "×"
...
function EmptyState({ title, description }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: title }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: description })]
	});
}
function SectionHeading({ title, subtitle }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "section-heading",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: title }), subtitle ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: subtitle }) : null]
	});
}
function Avatar({ name, photo, large = false }) {
	if (photo) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: classNames("avatar", large && "avatar--large"),
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: classNames("avatar", large && "avatar--large"),
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: getInitials(name) })
	});
}
function BrandLogo() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
...
function Field({ label, icon, className, ...inputProps }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
		...
		children: [label ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			...
			className: "field__surface",
			children: [icon ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "field__icon",
				children: icon
			}) : null, /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { ...inputProps })]
		})]
	});
}
function OptionCard({ label, selected, onClick }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
		...
		onClick,
		type: "button",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: label }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "option-card__radio" })]
	});
}
...
function getInitialSurveyStepIndex(survey) {
...
function buildVKName(profile) {
	if (!profile) return null;
	return [profile.first_name, profile.last_name].filter(Boolean).join(" ").trim() || null;
}
function matchesRecipeCatalogTab(recipe, tab) {
	if (tab === "Все рецепты") return true;
	if (tab === "Фастфуд") return recipe.cookMinutes <= 15 || recipe.tags.includes("Быстро");
	if (tab === "Овощи") {
		const haystack = [
			recipe.title,
			recipe.subtitle,
		...
		return recipe.tags.includes("Овощи") || recipe.category === "Салаты" || recipe.category === "Закуски" || haystack.includes("овощ") || haystack.includes("кабач") || haystack.includes("баклаж") || haystack.includes("капуст") || haystack.includes("тыкв");
	}
	if (tab === "Супы") return recipe.category === "Супы";
	return recipe.category === "Рыба";
}
function getRecipeScore(recipe, survey) {
	let score = 0;
	if (survey.diabetesType && recipe.safeFor.includes(survey.diabetesType)) score += 3;
	if (survey.therapy && recipe.therapyCompatibility.includes(survey.therapy)) score += 2;
	if (recipe.tags.includes("Низкий ГИ")) score += 2;
	if (survey.hypoglycemia === "Да, часто" && recipe.carbs >= 20 && recipe.carbs <= 32) score += 1;
	return score;
}
...
function buildChartPoints(entries, range) {
	const points = [...entries].sort((left, right) => new Date(left.timestamp).getTime() - new Date(right.timestamp).getTime()).map((entry) => ({
		label: range === "7d" ? formatWeekday(entry.timestamp) : new Date(entry.timestamp).getDate().toString(),
		value: entry.value
	}));
...
function metricEntryTitle(entry) {
	if (entry.type === "glucose") return "Глюкоза крови";
	if (entry.type === "weight") return "Вес";
	return "Активность";
}
...
function getRussianPlural(count, forms) {
	...
	if (remainder10 >= 2 && remainder10 <= 4 && (remainder100 < 12 || remainder100 > 14)) return forms[1];
	return forms[2];
}
function formatAge(age) {
	return `${age} ${getRussianPlural(age, [
		"год",
		"года",
		"лет"
	])}`;
...
function formatMetricValue(entry) {
	if (entry.type === "steps") {
		...
		return `${steps} ${getRussianPlural(steps, [
			"шаг",
			"шага",
			"шагов"
		])}`;
...
function buildPatientId(id) {
	return `MDN-${id.slice(0, 5).toUpperCase()}`;
}
function formatSavedRecipes(count) {
...
function getFirstName(name) {
	var _name$split$;
	return (_name$split$ = name.split(" ")[0]) !== null && _name$split$ !== void 0 ? _name$split$ : name;
}
function getInitials(name) {
	return name.split(" ").slice(0, 2).map((part) => {
		var _part$0$toUpperCase, _part$;
		return (_part$0$toUpperCase = (_part$ = part[0]) === null || _part$ === void 0 ? void 0 : _part$.toUpperCase()) !== null && _part$0$toUpperCase !== void 0 ? _part$0$toUpperCase : "";
	}).join("");
}
function shortenTherapy(value) {
	if (!value) return "—";
	if (value.includes("Инсулинотерапия")) return "Инсулин";
	if (value.includes("таблетки + инсулин")) return "Комбо";
	if (value.includes("таблетки")) return "Таблетки";
	if (value.includes("Ничего")) return "Без терапии";
...
function IconWrapper({ children, active = false, filled = false }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
		className: classNames("icon", active && "icon--active", filled && "icon--filled"),
		children
	});
}
...
function SearchIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(IconWrapper, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
		viewBox: "0 0 24 24",
		fill: "none",
...
function HeartIcon({ filled = false }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(IconWrapper, {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
			viewBox: "0 0 24 24",
			fill: "none",
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M12 20s-6.5-3.9-8.3-8A4.8 4.8 0 0 1 12 6a4.8 4.8 0 0 1 8.3 6c-1.8 4.1-8.3 8-8.3 8Z" })
		})
	});
}
function ArrowLeftIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
		viewBox: "0 0 24 24",
		fill: "none",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "m14.5 6.5-5 5 5 5" })
	});
}
function LockIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(IconWrapper, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("rect", {
			...
			height: "10",
			rx: "2"
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M8 10V8a4 4 0 0 1 8 0v2" })]
	}) });
}
function MailIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(IconWrapper, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("rect", {
			...
			height: "12",
			rx: "2"
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "m5.5 8 6.5 5 6.5-5" })]
	}) });
}
function UserIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(IconWrapper, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("circle", {
			...
			cy: "8",
			r: "3.2"
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M6.5 18.2c1.5-2.5 3.4-3.7 5.5-3.7s4 1.2 5.5 3.7" })]
	}) });
}
function ChevronRightIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
		viewBox: "0 0 24 24",
		fill: "none",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "m10 7 5 5-5 5" })
	});
}
function PlusIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
...
function ScaleIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("rect", {
			...
			height: "14",
			rx: "3"
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M12 9v4M8.5 10.5 12 9l3.5 1.5" })]
	});
}
function ActivityIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
		viewBox: "0 0 24 24",
		fill: "none",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: "M5 15h3l2-6 4 10 2-4h3" })
	});
}
//#endregion
//#region src/main.tsx
var geologicaFontFaces = `
@font-face {
...
function renderFatalScreen(error) {
    ...
    <div style="min-height:100vh;padding:24px;font-family:Geologica,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f5f4fb;color:#18141f;display:flex;align-items:center;justify-content:center;">
      <div style="max-width:360px;width:100%;background:#fff;border:1px solid rgba(78,63,255,.12);border-radius:24px;padding:24px;box-shadow:0 24px 48px rgba(31,19,74,.12);">
        ...
        <pre style="margin:0;white-space:pre-wrap;word-break:break-word;background:#f3f1ff;border-radius:16px;padding:14px;font-size:13px;line-height:1.45;color:#30276b;">${escapeHtml(normalizeError(error))}</pre>
      </div>
    </div>
  `;
}
window.addEventListener("error", (event) => {
	var _event$error;
	renderFatalScreen((_event$error = event.error) !== null && _event$error !== void 0 ? _event$error : event.message);
});
window.addEventListener("unhandledrejection", (event) => {
	renderFatalScreen(event.reason);
});
injectGeologicaFonts();
initializeVKBridge().catch((error) => {
	renderFatalScreen(error);
});
try {
	import_client.createRoot(document.getElementById("root")).render((0, import_react.createElement)(App));
} catch (error) {
	renderFatalScreen(error);
}
//#endregion
